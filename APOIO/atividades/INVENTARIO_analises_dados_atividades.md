...
| Linear simples | ✅ | `camarao_vannamei_biometria`; `cangulo_crescimento` | ❌ |
...
| Linear múltipla *(em dev)* | ✅ | `truta_riacho_crescimento`; `tilapia_microalgas` | ❌ |
...
- **Atividades hoje (5):** cobrem só os **não paramétricos** — qui-quadrado, Mann–Whitney, Wilcoxon, Kruskal–Wallis. Toda a régua de descrição, regressão, paramétricos, multivariada, mapas e séries **ainda não tem atividade**.
...
- **Próximas atividades naturais:** ANCOVA (já tem `bagley` no pacote → dá um Excel externo distinto), regressão (linear/não linear), PCA/HCA e teste t — cada uma sai do molde.
