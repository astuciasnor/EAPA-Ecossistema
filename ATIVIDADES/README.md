...
`provisorios/` preserva os downloads originais, metadados e verificações de
cada candidato. Um arquivo só segue para `dados/` quando fonte, licença, unidade
observacional e uso didático foram conferidos.

O registro das fontes aprovadas fica em `dados/REGISTRO_FONTES_DADOS_EAPA.xlsx`:
a aba **fontes** é o registro em si e a aba **resumo** cruza cada análise do
catálogo com o conjunto canônico, o arquivo externo e a atividade, para mostrar de
relance o que já tem dados e o que falta.

Para buscar e revisar novos conjuntos com poucos passos, consulte o
[guia rápido de busca](skills/eapa-curadoria-atividades/GUIA_RAPIDO_BUSCA.md).
...
Para localizar o par de teste de cada análise, consulte a
[matriz análise → pacote → arquivo externo](skills/eapa-curadoria-atividades/references/matriz-analise-pacote-arquivo.md).
...
| 6 | [Formalina e remoção semanal no salvelino](atividade_anova_dois_fatores_salvelino_formalina.md) | ANOVA a dois fatores (interação) | `dados/salvelino_formalina_remocao.xlsx` (Olk et al.; DataverseNO; CC BY 4.0) · banco externo | Intermediário |
| 7 | [Dieta e temperatura em Gammarus](atividade_interpretacao_fatorial_gammarus.md) | Gráficos e interpretação de experimento fatorial | `dados/gammarus_dieta_temperatura_resumo.xlsx` (Ribes-Navarro et al.; Frontiers; CC BY) · banco externo | Introdutório–intermediário |
| 8 | [Cravo e lidocaína na anestesia da tilápia](atividade_anova_um_fator_tilapia_anestesia.md) | ANOVA de um fator | `dados/anova_tilapia_anestesia.xlsx` (Martínez; Mendeley Data; CC BY 4.0) · banco externo | Introdutório–intermediário |
| 9 | [O raio do otólito prevê o comprimento do peixe?](atividade_regressao_linear_simples_otolito_comprimento.md) | Regressão linear simples | `dados/regressao_otolito_comprimento.xlsx` (Morat et al.; figshare; CC BY 4.0) · banco externo | Intermediário |
| 10 | [O mercúrio no peixe depende da idade, do tamanho — ou do que ele come?](atividade_regressao_multipla_mercurio_lago_escravo.md) | Regressão linear múltipla | `dados/mercurio_lago_escravo.xlsx` (Rohonczy et al.; Dryad; CC0) · banco externo | Intermediário–avançado |

## Atividades reservadas (triagem inicial)

Estas atividades foram desmembradas de abas ainda nao incorporadas ao EAPADados. Sao rascunhos didaticos: revisar fonte, licenca e rubrica antes de distribuicao publica. Para não misturá-las às dez atividades consolidadas, os enunciados e planilhas ficam em `_preliminares/`.
